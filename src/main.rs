...
fn verify_downloaded_binary(
    ...
    verify_binary_checksum(binary, asset_name, &sums)?;
    println!("{} Checksum verified (sha256).", icon_ok().green());
    Ok(())
}

fn verify_binary_checksum(binary: &std::path::Path, asset_name: &str, sums: &str) -> Result<(), String> {
    let expected = sha256sums_lookup(sums, asset_name).ok_or_else(|| {
        format!(
            "{} has no unique valid entry in SHA256SUMS — refusing to install.",
            asset_name
        )
    })?;
...
fn sha256sums_lookup(sums: &str, asset_name: &str) -> Option<String> {
    let mut matches = sums.lines().filter_map(|line| {
        ...
        (name == asset_name).then_some((hash, fields.next().is_none()))
    });
    let (hash, exact) = matches.next()?;
    if matches.next().is_some() || !exact || hash.len() != 64 || !hash.chars().all(|c| c.is_ascii_hexdigit()) {
        return None;
    }
    ...
    if console::is_windows() {
        ...
        let out = std::process::Command::new(setup::windows_powershell())
            .args([
                "-NoProfile",
                "-Command",
                "(Get-FileHash -Algorithm SHA256 -LiteralPath $env:TINA4_VERIFY_FILE).Hash",
            ])
            .env("TINA4_VERIFY_FILE", path)
            .output()
            .ok()?;
        ...
        let s: String = String::from_utf8_lossy(&out.stdout)
            .trim()
            .to_lowercase();
        return (s.len() == 64 && s.chars().all(|c| c.is_ascii_hexdigit())).then_some(s);
    }
...
mod tests {
    ...
    #[test]
    fn updater_hashes_real_bytes_and_rejects_ambiguous_manifests() {
        let path = std::env::temp_dir().join(format!("tina4-sha-'quoted-{}.bin", std::process::id()));
        std::fs::write(&path, b"abc").unwrap();
        assert_eq!(super::sha256_of_file(&path).as_deref(), Some("ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad"));
        let valid = "ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad  asset\n";
        assert!(super::verify_binary_checksum(&path, "asset", valid).is_ok());
        std::fs::write(&path, b"tampered").unwrap();
        assert!(super::verify_binary_checksum(&path, "asset", valid).is_err());
        assert!(super::verify_binary_checksum(&path, "asset", "").is_err());
        std::fs::remove_file(&path).unwrap();
        assert_eq!(super::sha256_of_file(&path), None);
        let h = "a".repeat(64);
        assert_eq!(sha256sums_lookup(&format!("{h}  asset\n{h}  asset\n"), "asset"), None);
        assert_eq!(sha256sums_lookup(&format!("{}  asset\n", "z".repeat(64)), "asset"), None);
        assert_eq!(sha256sums_lookup(&format!("{h}  asset extra\n"), "asset"), None);
    }
